		...
		err := ngFilter.ForEach(cfg.NodeGroups, func(_ int, ng *api.NodeGroup) error {
			...
			if ng.IAM == nil || ng.IAM.InstanceRoleARN == "" {
				if err := ctl.GetNodeGroupIAM(stackManager, cfg, ng); err != nil {
					logger.Warning("getting instance role ARN for node group %q", ng.Name)
					return nil
				}
			}
...
func collectMissingNodeGroups(ngFilter *cmdutils.NodeGroupFilter, stackManager *manager.StackCollection, nodeGroups *[]*api.NodeGroup) error {
	if clusterConfigFile != "" {
		...
		for _, s := range stacks {
			if stackManager.StackStatusIsNotReady(s) {
				logger.Warning("ignoring stack %q (status: %q)", *s.StackName, *s.StackStatus)
				continue
			}
		...
		for _, localNodeGroup := range *nodeGroups {
			...
			if !remote.Has(localNodeGroup.Name) {
				logger.Info("nodegroup %q present in the config file %q, but missing in the cluster", localNodeGroup.Name, clusterConfigFile)
				ngFilter.AppendExcludeNames(localNodeGroup.Name)
			} else if deleteOnlyMissingNodeGroups {
				ngFilter.AppendExcludeNames(localNodeGroup.Name)
			}
		}
		...
		for remoteNodeGroupName := range remote {
			if !local.Has(remoteNodeGroupName) {
				logger.Info("nodegroup %q present in the cluster, but missing from the config file %q", remoteNodeGroupName, clusterConfigFile)
				if deleteOnlyMissingNodeGroups {
					...
					*nodeGroups = append(*nodeGroups, &api.NodeGroup{Name: remoteNodeGroupName})
					...
					if ngFilter.Match(remoteNodeGroupName) {
						ngFilter.AppendIncludeNames(remoteNodeGroupName)
					}
				}
			}
		}
